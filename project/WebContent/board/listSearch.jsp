...
<!-- 상단  -->
    <div class="container">
      ...
      <div class="my-5">
        <h1 class="h3 font-weight-normal"> 검색 : 
        <%
        if(type==0){
            out.println("문의사항");
        }else if(type==1){
        ...
        %>
        </h1><!--         <img src="../assets/img/logo/logo.png" alt="" width="120" height="120">
 -->      </div>
...
<!-- 게시판 시작  -->
      <div class="col-lg-10 table-responsive-sm mx-auto">
<!--  게시판 검색 및 정보 -->
    <div id="searchInfo">

        <!--  검색화면이니 검색창 없앰-->
        <div id="table_search" class="float-right"> 
                <div id="btnList" class="float-right row mr-2 d-block"> 
                <input type="button" value="목록" class="btn btn-outline-primary btn-sm" onclick="location.href='list.jsp?type=<%=type%>'">
                </div>
        </div>
 ...
 <!--  게시판 리스트 -->
    ...
    %>
        <tr onclick ="location.href='content.jsp?num=<%=bb.getNum() %>&pageNum=<%=pageNum %>&type=<%=type%>'">
            <td><%= num-- %></td>
            <td class="text-left"><%= bb.getSubject()%>
                    <%
            if(bb.getFile()!=null){
                                     ...
                                     <div class="row">
                                         <figure class="figure">
                                           <a href="content.jsp?num=<%=bb.getNum() %>&pageNum=<%=pageNum %>&type=<%=type%>">
                                            ...
                                            <figcaption class="figure-caption"><h5><%=bb.getSubject() %></h5></figcaption>
                                         </figure>
                                     </div>
                            <%--         <div class="row">
